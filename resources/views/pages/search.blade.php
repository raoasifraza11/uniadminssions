				...
				<div class="Page-title">
					<div class="col-md-12 text-center">
						<div class="title-text">
							<h6>Great Service</h6>
							<h2 class="page-title">Search result found {{ $data['institutes']->count() }} Institutes</h2>
						</div>
					</div>
					<div class="col-md-12 text-center">
								...
								<div class="filter-option-content">
										...
										<div class="restaurant-service-filter">
											<ul class="list-unstyled">
												@foreach(\App\Category::all()->sortBy('name') as $category)
													<li>
														<input type="checkbox" name="category" id="{{ $category->name }}" value="{{ $category->id }}" {{ (($data['category_keyword'] == $category->name)?'checked':'') }}>
									...
									<div class="restaurant-cuisines-section">
										...
										<div class="restaurant-cuisines-filter">
											<ul class="list-unstyled">
												<li>
													<input type="checkbox" value="" name="city" id="allCity">
													<label for="allCity"><i class="icon-check custom-check"></i> View All</label>
												</li>
												@foreach(\App\City::all()->sortBy('name') as $city)
													<li>
														<input type="checkbox" value="{{ $city->id }}" name="city" id="{{ $city->name }}" {{ (($data['city_keyword'] == $city->name)?'checked':'') }}>
